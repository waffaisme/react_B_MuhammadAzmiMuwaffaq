...
export default function FetchDataWithAxios() {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://jsonplaceholder.typicode.com/posts', formData)
      .then((response) => {
        setResponseMessage('Post ditambahkan')
        setPosts((prevPosts) => [response.data, ...prevPosts])
        setFormData({title: '', body: ''})
      })
        .catch((error) => {
          setResponseMessage('Error saat menambahkan post baru')
          console.log('Cant post to server ', error)
        })
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-100 p-6">
      ...
      <div className="w-full max-w-lg bg-white p-6 rounded-lg shadow-lg mb-8">
        ...
        {responseMessage && (
          <div className="mt-4 text-sm text-center text-sky-600">{responseMessage}</div>
        )}
      </div>
